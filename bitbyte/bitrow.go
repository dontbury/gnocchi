...
func (b *BitRow) set(index, shift, size int, value int64) error {
	if index < 0 {
		...
		return fmt.Errorf("bitbyte.BitRow.set value(%d) mask(%d) shift(%d) size(%d) is too large", value, mask, shift, size)
	} else {
...
func (b *BitRow) get(index, shift, size int) (int64, error) {
	if index < 0 {
		...
		return 0, fmt.Errorf("bitbyte.BitRow.Get shift,size(%d) is too large", shift+size)
	}
	if index < len(b.Body) {
