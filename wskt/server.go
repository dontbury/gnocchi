...
func (s *Server) Run() {
	for {
		...
		if err := s.exS.ReceiveChannel(&buf); err != nil {
			log.Printf("wskt.Server.Run:ExServer.ReceiveChannel failure buf:%v.\n\t%v", buf, err)
		}
	}
}
...
func (s *Server) SendServerBroadcast(wsBuf *WSBuf) {
	...
	if buf, err := wsBuf.GetSendBuf(); err == nil {
		...
		log.Printf("wskt.Server.SendServerBroadcast:WSBuf.GetSendBuf failure wsBuf:%v.\n\t%v", wsBuf, err)
	}
}
...
func (s *Server) ClientLoop(check, send *WSBuf) (int, error) {
	...
	for i, c := range s.clients {
		if val, err := c.exC.CheckSend(check); err != nil {
			...
			if val > 0 && send != nil {
				if buf, err := send.GetSendBuf(); err == nil {
					c.send <- *buf
				} else {
					return 0, fmt.Errorf("wskt.Server.ClientLoop:WSBuf.GetSendBuf failure send:%v.\n\t%v", send, err)
				}
			}
			sum += val
		}
