...
func (con *Conn) AddOption(option interface{}) {
	if option != nil {
		con.Options = append(con.Options, option)
	}
}

// 引数をMapで渡してリダイレクトする
func (con *Conn) RedirectAddOption(w http.ResponseWriter, r *http.Request, path string, option interface{}) {
	if option != nil {
		con.AddOption(option)
	}
	http.Redirect(w, r, path, 302 /* Found 一時的な移動 キャッシュなし */)
}
...
func (accs *Accounts) Connection(w http.ResponseWriter, r *http.Request) *Conn {
	...
	con := &Conn{Sid: sid, Acc: nil, Date: time.Now(), Options: []interface{}{}}
	accs.AppendConn(con)
